...
//			}
	...
	<div class="Description">
		...
		<?php
				...
				$user_id=time(); //pairnei to time stamp pou einai monadiko gia na ginei h onomasia twn arxeiwn
				$target = "./results/"; 
				$target = $target . basename( $_FILES['uploaded']['name']) ; 
				$ok=1; 
				//This is our size condition 
				if ($_FILES['uploaded']['size'] > 52000000) { #an to arxeio einai megalutero apo 30mb den to shkwnei (ligo megalytero apo to mege8os ou an8rwpinou proteomatos)
					echo  "<table border=\"0\" align=\"center\"><tr><td>Your file is too large.";
					$ok=0;
					exit;
				} 
				...
				//If everything is ok we try to upload it 
					...
					//if(move_uploaded_file($_FILES['uploaded']['tmp_name'], $target)) { 
						//echo "<br>The file ".basename( $_FILES['uploaded']['name']). " has been uploaded. Please wait for the results...<br>";
							echo "<div class=\"Code\" style=\"width:80%;margin: 0 auto\">";
								//echo "<center>";
								echo "<div id=\"myObj1\" style=\"position:inline\">";
									echo "<p><u>Do not refresh this page, as it may take more than 20 minutes</u>";
									echo "<br>Your user id is <strong><em>$user_id</em></strong>. <i>Please refer to the <a href='manual.html' target='_blank'>manual</a> to see how to retrieve a text file with your results after the job is finished</i></p>";
									echo "<img src=\"wait4.gif\" alt=\"wt\" height=\"100\" width=\"100\">";
								echo "</div>";

//		echo <!--"<div id="myObj2" style="visibility:hidden;position:absolute">";
//		echo "<img src="complete.png" alt="cm">";
//		echo "</div> -->";
								//echo "</center>";
//		echo "<!--<h4><strong>"; -->
...
//						echo filesize($target);
	...
	if($ok==1){
				...
				$input_file = "./results/$user_id.pdb";
...
{
	login("http://aias.biol.uoa.gr/AMYLPRED2/login.php","email=annarerra%40biol.uoa.gr&password=oi4tvst23"); //login to amylpred2
    ...
    $file_mid1 = fopen($filename_mid,"r")or die("Couldn't open $filename");
    ...
    $file_amylresult1 = fopen($filename_amylresult,"r")or die("Couldn't open $filename");
	while(! feof($file_amylresult1))
    {
		$a=fgets($file_amylresult1); //. "<br />" ανα γραμμή το κείμενο αποθηκεύεται στην $a
...
function print_intervals(array $array_up,array $array_down,$sequence,$name)
	{
...
$file1 = fopen($filename,"r")or die("Couldn't open $filename"); //άνοιγμα αρχείου
...
{	
	...
	if (preg_match("/^ATOM/",$a))
	{
		echo "Its ok you have a pdb file";
		//$help_me=1;
		exit;
	}
	}
	...
	if ($help===$hd)
	{
		exit("<br><br>Your file is not a pbd file");
	}
	$hd++;
}*/
...
while(! feof($file1))
 ...
 if ($help == $hd)
	{
		exit ("Your file is not a pdb file, Please insert a valid pdb file");
	}
 if ($help_h == $hd)
	{
...
//-------------delete CDRS----------------
for ($i=$start_h1-1; $i<=$finish_h1+1; $i++)
{
...
system ("./foldx --command=PositionScan --pdb=$user_id.pdb --positions=$positions --ionStrength=$ionstr --pH=$ph --output-dir ./results/ > foldxoutput_$user_id.txt");
sleep(1);
...
//------Heavy--------
	...
	$old_fasta_file = fopen("./results/".$user_id."old_fasta_file.txt","w")or die("Couldn't open $old_fasta_file"); 
    //echo $fasta_id;
	...
	echo "<br><br><span style='margin:10px;' ><a href=\"results/".$user_id."old_fasta_file.txt\" target=\"_blank\">Download FASTA before substitutions</a>";	
	//echo $old_fasta_file;
